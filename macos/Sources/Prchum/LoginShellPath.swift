...
/// Homebrew, npm, cargo, or go is invisible to a plain `Process` —
/// including `gh`, `glab`, and `fj`, the CLIs every forge call runs
/// through. Asking the user's login shell for its PATH (with a short
/// timeout, in case a shell profile hangs) and appending a few
/// conventional tool directories fixes that for the whole process.
func adoptLoginShellPath() {
    ...
    // first, so a tool the user installed wins over an older system copy
